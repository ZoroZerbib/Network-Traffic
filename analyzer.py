from checks import check_internal_address, check_sensitive_port, check_package_size, check_time, check_hour_rescue, \
    package_size_conversion, running_tests_line
...
def analyzer_processing_log(data_list):
    h = list(map(lambda line: (line, running_tests_line(line)), data_list))
    d = list(filter(lambda result: len(result[1]) > 0, h))
    return d
